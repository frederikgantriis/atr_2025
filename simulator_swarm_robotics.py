...
logging.basicConfig(filename='dispersion-random-noisy-with-light-source.log', encoding='utf-8', level=logging.DEBUG)
...
class Robot:
    ...
    def robot_controller(self):
        ...
        # light_source_intensity = 1.0 - self.light_intensity
        # if self.light_intensity > 0.1:
        #     if s["intensity"] > ms["intensity"] and s["intensity"] < 0.5:
        #         ms = s
        # else:
        ...
        if len(self.rab_signals) > 0:
            ...
            if self.light_intensity < ms["intensity"]:
